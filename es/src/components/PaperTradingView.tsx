...
import { DollarSign, TrendingUp, History, Database, Wallet, ShieldCheck } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            {hasAlpaca ? 'Alpaca Paper Trading Account Portfolio' : 'Paper Trading Portfolio & Execution Engine'}
          </h2>
          <p className="text-xs text-gray-400">
            {hasAlpaca
              ? 'Synced directly with Alpaca Paper Trading REST API (https://paper-api.alpaca.markets).'
              : 'Real-time simulated execution with realistic order fills, slippage, and equity tracking.'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`px-3 py-1 rounded-xl text-xs font-bold border flex items-center gap-1.5 ${
            ...
            {hasAlpaca ? 'Alpaca API Connected' : 'Local Engine Active'}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-panel p-5 rounded-2xl border border-gray-800">
          <span className="text-xs text-gray-400">{hasAlpaca ? 'Alpaca Account Equity' : 'Account Equity'}</span>
          <p className="text-2xl font-black text-white mt-1">${portfolio.equity.toLocaleString()}</p>
          <span className="text-[11px] text-gray-500">Cash: ${portfolio.balance.toLocaleString()}</span>
        </div>

        <div className="glass-panel p-5 rounded-2xl border border-gray-800">
          <span className="text-xs text-gray-400">{hasAlpaca ? 'Alpaca Buying Power' : 'Buying Power'}</span>
          <p className="text-2xl font-black text-emerald-400 mt-1">${(portfolio.freeMargin || portfolio.equity).toLocaleString()}</p>
          <span className="text-[11px] text-gray-500">Available for AI Trades</span>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-gray-800">
          ...
          <span className="text-[11px] text-gray-500">Today P&L: ${portfolio.dailyPnL.toFixed(2)}</span>
        </div>
