...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // KPI Metrics (using live portfolio if available, falling back to rich defaults matching the mockup)
  ...
  const openPositionsExposure = 8250.45;
  const winRate = portfolio?.winRate ? portfolio.winRate * 100 : 64.8;
  const totalTradesCount = portfolio?.totalTrades ?? 37;
  const profitFactor = portfolio?.profitFactor ?? 1.72;
  const maxDrawdown = portfolio?.maxDrawdownPercent ?? 3.21;
  ...
  const agentPerformanceList = [
    { name: 'Regime Agent', icon: Compass, accuracy: 71.2, impact: '+0.42R', alignment: 78, positive: true },
    { name: 'Technical Agent', icon: Cpu, accuracy: 74.6, impact: '+0.58R', alignment: 82, positive: true },
    { name: 'Liquidity Agent', icon: Droplets, accuracy: 64.1, impact: '+0.31R', alignment: 68, positive: true },
    ...
    { name: 'Volatility Agent', icon: Activity, accuracy: 53.2, impact: '-0.08R', alignment: 54, positive: false },
  ...
  const recentTradesList = [
    ...
    { time: 'Yesterday 22:41', symbol: 'XRPUSDT', side: 'LONG', r: '+1.05R', pnl: '+$52.18', outcome: 'WIN', outcomeColor: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' },
    { time: 'Yesterday 20:13', symbol: 'ETHUSDT', side: 'LONG', r: '-0.63R', pnl: '-$63.21', outcome: 'LOSS', outcomeColor: 'bg-rose-500/20 text-rose-400 border-rose-500/30' },
  ];
  ...
  const marketList = [
    {
      ...
      high24h: snapshot?.symbol === 'BTCUSDT' ? snapshot.high24h : 65150.0,
      low24h: snapshot?.symbol === 'BTCUSDT' ? snapshot.low24h : 63100.5,
      volume: '$24.8B',
      sparkline: [63100, 63400, 63800, 63600, 64100, 63900, 64250],
    },
    {
      symbol: 'ETHUSDT',
      price: snapshot?.symbol === 'ETHUSDT' ? snapshot.price : 3142.25,
      change24h: snapshot?.symbol === 'ETHUSDT' ? snapshot.change24h : 2.18,
      high24h: snapshot?.symbol === 'ETHUSDT' ? snapshot.high24h : 3210.8,
      low24h: snapshot?.symbol === 'ETHUSDT' ? snapshot.low24h : 3020.1,
      volume: '$12.6B',
      sparkline: [3020, 3060, 3100, 3080, 3120, 3110, 3142],
    },
    {
      symbol: 'SOLUSDT',
      price: snapshot?.symbol === 'SOLUSDT' ? snapshot.price : 152.68,
      change24h: snapshot?.symbol === 'SOLUSDT' ? snapshot.change24h : 3.21,
      high24h: snapshot?.symbol === 'SOLUSDT' ? snapshot.high24h : 155.3,
      low24h: snapshot?.symbol === 'SOLUSDT' ? snapshot.low24h : 146.22,
      volume: '$2.1B',
      sparkline: [146, 148, 149, 147, 151, 150, 152.68],
    },
    {
      symbol: 'XRPUSDT',
      price: snapshot?.symbol === 'XRPUSDT' ? snapshot.price : 0.5432,
      change24h: snapshot?.symbol === 'XRPUSDT' ? snapshot.change24h : 0.87,
      high24h: snapshot?.symbol === 'XRPUSDT' ? snapshot.high24h : 0.5521,
      low24h: snapshot?.symbol === 'XRPUSDT' ? snapshot.low24h : 0.531,
      volume: '$1.2B',
      sparkline: [0.531, 0.534, 0.538, 0.536, 0.54, 0.541, 0.5432],
    },
  ];
