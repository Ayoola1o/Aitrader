...
const EMPTY_BINS: PnLBin[] = [
  { label: '<-2R', count: 0, type: 'loss' },
  { label: '-2R to -1R', count: 0, type: 'loss' },
  { label: '-1R to 0', count: 0, type: 'loss' },
  { label: '0 to 1R', count: 0, type: 'win' },
  { label: '1R to 2R', count: 0, type: 'win' },
  { label: '2R to 3R', count: 0, type: 'win' },
  { label: '>3R', count: 0, type: 'win' },
];
...
export const PnLHistogram: React.FC<PnLHistogramProps> = ({
  ...
  const highestBinCount = Math.max(...bins.map((b) => b.count), 1);
  const maxCount = Math.max(highestBinCount, 6);
